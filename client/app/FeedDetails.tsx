...
const FeedDetails = () => {
  ...
  const toggleLike = async () => {
    ...
    if (liked) {
      const success = await FeedAPIClient.unlikePost(
        Number(post.id),
        currentUser.id
      )
      if (success) {
      ...
      const success = await FeedAPIClient.likePost(
        Number(post.id),
        currentUser.id
      )
      if (success) {
  ...
  const userInitial = post.author_name
      ...
      <ScrollView className="p-4">
        ...
        <View className="flex-row justify-between items-center my-4">
          <View className="flex-row items-center space-x-2">
            <View
              className={`w-10 h-10 rounded-full ${avatarColor} flex items-center justify-center mr-2`}
            >
        ...
        <View className="mt-6 p-4 border rounded-lg border-gray-300">
          <View className="flex-row justify-between">
            ...
            <View
              className={`w-8 h-8 rounded-full ${avatarColor} flex items-center justify-center`}
            >
              <Text className="text-white font-bold">{userInitial}</Text>
            </View>
          </View>
              ...
              <View key={c.id} className="py-3 border-b border-gray-200">
                ...
                <View className="flex-row justify-between items-center">
                  <View className="flex-row items-center space-x-2">
                    {/* Avatar */}
                    <View
                      className={`w-8 h-8 rounded-full ${color} flex items-center justify-center mr-2`}
                    >
