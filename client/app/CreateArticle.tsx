...
//   const [coverImage, setCoverImage] = useState<{ uri: string; file?: File } | null>(null);
...
//         const file = new File([blob], result.name, { type: result.mimeType || "image/jpeg" });

//         setCoverImage({ uri: result.uri, file });
//       } else {
...
//           const blob = await response.blob();
//           const file = new File([blob], asset.fileName || "cover.jpg", { type: asset.mimeType || "image/jpeg" });

//           setCoverImage({ uri: asset.uri, file });
//         }
//       }
//     } catch (error) {
...
//       let result;
//       if (Platform.OS === "web") {
//         result = await DocumentPicker.getDocumentAsync({ type: type === "image" ? "image/*" : "video/*" });
//         if (result.type === "cancel") return;

//         const response = await fetch(result.uri);
//         const blob = await response.blob();
//         const file = new File([blob], result.name, { type: result.mimeType || "image/jpeg" });
//         handleUpdateBlock(index, type, { uri: result.uri, file });
//       } else {
//         result = await ImagePicker.launchImageLibraryAsync({
...
//           handleUpdateBlock(index, type, { uri: asset.uri, file });
//         }
//       }
//     } catch (error) {
...
//   const handleUpdateBlock = (index: number, type: string, value?: { uri: string; file: File }) => {
...
//     const formattedContents = contentBlocks.map((block, index) => ({
//       content_type: block.type,
//       content_data: block.value?.uri || "",
//       position: index + 1,
//     }));

//     const articlePayload = { title, category, cover_image: coverFileName, contents: formattedContents };
//     try {
...
//         {coverImage ? (
//             <Image source={{ uri: coverImage.uri }} style={{ width: "100%", height: 150, borderRadius: 5 }} />
//         ) : (
//             <>
...
const CreateArticle = () => {
  ...
  const uploadImageToBackend = async (uri: string) => {
    try {
      ...
      const formData = new FormData();
      formData.append("body", JSON.stringify({})); // Required by backend

      // Append image file as Blob
      formData.append("file", {
        uri,
        name: "upload.jpg",
        type: "image/jpeg",
        blob, // Ensure Firebase receives actual file data
      } as any);
  ...
  const pickMedia = async (index: number, type: string) => {
    try {
      ...
      if (!result.canceled && result.assets.length > 0) {
        const uri = result.assets[0].uri;
        const uploadedFileName = await uploadImageToBackend(uri);
        if (uploadedFileName) {
  ...
  const handlePublish = async () => {
    ...
    const formattedContents = contentBlocks.map((block, index) => ({
      content_type: block.type,
      content_data: block.value,
      position: index + 1,
    }));

    const articlePayload = { title, author_id: 1, category, cover_image: coverFileName, contents: formattedContents };
