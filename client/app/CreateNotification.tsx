...
import NotificationAPIClient, {
  SendEmailInfo,
} from '@/APIClients/NotificationAPIClient'
...
const CreateNotification = () => {
  ...
  const handleSendNotification = async () => {
    ...
    const emailData: SendEmailInfo = {
      subject: subject.trim(),
      body: content.trim(),
    }
    console.log('Sending Notification:', { subject, content })
    try {
      await NotificationAPIClient.send_email({
        entityData: emailData,
      })
    } catch (error) {
      console.error('Error sending email notifs', error)
    }
    setShowModal(true)
  }
        ...
        <View className="flex-1 justify-center items-center bg-black/50">
          <View className="bg-white p-6 rounded-lg w-3/4">
            ...
            <Text className="text-gray-600 mt-2">
              Users with notifications enabled will be emailed.
            </Text>
            <TouchableOpacity
