...
const Feed = () => {
  ...
  const toggleLike = async (postId: number, event?: any) => {
    ...
    if (isCurrentlyLiked) {
      // Removing like
      updatedPost.usersWhoHaveLiked = (post.usersWhoHaveLiked || []).filter(
        (id) => id !== userId
      )
      updatedPost.likesCount =
        (post.likesCount || 0) > 0 ? (post.likesCount || 0) - 1 : 0
    } else {
      // Adding like
    ...
    return (
      ...
      >
        ...
        <View className="flex-row items-center p-3 border-b border-gray-200">
          <View
            className={`w-10 h-10 rounded-full ${avatarColor} flex items-center justify-center mr-6`}
          >
        ...
        >
          ...
          <View className="mt-3 flex-row justify-end">
            <View className="bg-purple-100 rounded-full px-4 py-2 flex-row items-center">
              <Text className="text-purple-700 font-medium mr-1">
                Read More
              </Text>
  ...
  const renderEmptyComponent = () => (
    <View className="flex-1 justify-center items-center py-20">
      ...
      <Text className="text-gray-400 font-medium mt-4 text-lg">
        No posts yet
      </Text>
